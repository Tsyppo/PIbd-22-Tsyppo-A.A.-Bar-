...
namespace AbstractBarDatabaseImplement.Implements
{
    public class CocktailStorage : ICocktailStorage
    {
        ...
        public CocktailViewModel GetElement(CocktailBindingModel model)
        {
            ...
            .Include(rec => rec.CocktailComponents)
            .ThenInclude(rec => rec.Component)
            .FirstOrDefault(rec => rec.CocktailName == model.CocktailName ||
            rec.Id == model.Id);
            return Cocktail != null ? CreateModel(Cocktail) :
           null;
        }
        ...
        public void Insert(CocktailBindingModel model)
        {
            ...
            try
            {
                Cocktail g = new Cocktail
                {
                    CocktailName = model.CocktailName,
                    Price = model.Price
                };
                context.Cocktails.Add(g);
                context.SaveChanges();
        ...
        public void Update(CocktailBindingModel model)
        {
            ...
            try
            {
                var element = context.Cocktails.FirstOrDefault(rec => rec.Id ==
                model.Id);
                if (element == null)
                {
        ...
        AbstractBarDatabase context)
        {
            ...
            // добавили новые
            foreach (var gt in model.CocktailComponents)
            {
        ...
        private static CocktailViewModel CreateModel(Cocktail Cocktail)
        {
            ...
            .ToDictionary(recGT => recGT.ComponentId,
            recGT => (recGT.Component?.ComponentName, recGT.Count))
            };
        }
    }
}